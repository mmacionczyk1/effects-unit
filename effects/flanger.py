...
class Flanger(AudioEffect):
    def __init__(self, rate_hz: float = 0.2, base_delay_ms: float = 20.0, mix_parameter: float = 0.5, feedback: float = 0.5, depth_ms: float = 2.0):
    ...
    def process(self, input_data: np.ndarray, sample_rate: int) -> np.ndarray:
        ...
        for i in range(N):
            ...
            if idx2 >= 0:
                delayed_sample = (1.0 - frac_delay) * feedback_loop[idx1] + frac_delay * feedback_loop[idx2]
            elif idx1 >= 0:
                delayed_sample = feedback_loop[idx1]
            else:
                delayed_sample = 0.0
            feedback_loop[i] = input_data[i] + self.feedback * delayed_sample
...
def main():
    ...
    if len(data.shape) > 1:
        data = data[:, 0]
    ovd = Flanger(rate_hz=0.5, depth_ms=2.0, base_delay_ms=1.0, mix_parameter=0.5, feedback=0.7)
